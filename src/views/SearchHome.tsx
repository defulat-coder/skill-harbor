...
function SearchHomeContent({ index }: { index: ReturnType<typeof useSkillIndex> }) {
    ...
    (result?.hits ?? []).reduce<
      Record<
        string,
        {
          skillId: string;
          name: string;
          sources: { hit: SkillSearchResult["hits"][number]; index: number }[];
        }
      >
    >((all, hit, hitIndex) => {
      const group = (all[hit.skill_id] ??= { skillId: hit.skill_id, name: hit.name, sources: [] });
      group.sources.push({ hit, index: hitIndex });
      return all;
    }, {}),
  );
  ...
  useEffect(() => {
    recentSearch = { root: status?.root, query, result, answer };
  }, [status?.root, query, result, answer]);
  ...
  async function generateAnswer(found: SkillSearchResult, token: number) {
    ...
    try {
      ...
      if (mounted.current && token === request.current)
        setAnswerError(getErrorMessage(error, "中文回答生成失败，检索结果仍可查看"));
    }
  }
  async function search() {
    ...
    try {
      ...
      if (mounted.current && token === request.current)
        setError(getErrorMessage(error, "技能检索失败，请重试"));
    } finally {
      if (mounted.current && token === request.current) {
  ...
  async function retryAnswer() {
    ...
    const token = ++request.current;
    try {
      await generateAnswer(result, token);
    } finally {
      if (mounted.current && token === request.current) {
        working.current = false;
        setPhase("idle");
    ...
    <div className={styles.page}>
      <section className={styles.hero} aria-labelledby="skill-question-title">
        ...
        >
          ...
          >
            <label className={styles.inputLabel} htmlFor="skill-question">
              描述你的需求
            </label>
            <textarea
              ref={textarea}
              ...
              placeholder="例如：有哪些技能能帮我检查代码？应该怎么使用？"
              rows={4}
            ...
            {beamPhase === "idle" ? null : (
              <div
                ...
                onAnimationEnd={(event) => {
                  if (event.animationName.includes("beamFadeOut")) setBeamPhase("idle");
                }}
              >
          ...
          <div className={styles.directory}>
            ...
            <span title={status?.root}>全局技能库</span>
            <Link to="/library">浏览技能库</Link>
          </div>
        </form>
          ...
          <span role="status">
            {building
              ...
              : statusLoading
                ? "正在检查索引…"
                : status?.ready
                  ? `索引就绪 · ${status.files} 个文件`
                  : "尚未建立可用索引"}
          </span>
          <Link to="/search-index" className={styles.indexLink}>
            索引管理 <ArrowRight size={14} aria-hidden />
          </Link>
        </div>
        {!status?.ready && !statusLoading && !building && (
          <p className={styles.hint}>请先到索引管理建立索引，再用中文提问。</p>
        )}
        {(statusError || status?.error) && (
          <div role="alert" className={styles.error}>
            {statusError || status?.error}
            <Button disabled={busy || statusLoading || building} onClick={() => index.refresh()}>
              重新检查
            </Button>
          </div>
        )}
      ...
      {result && (
        <section className={styles.results} aria-label="技能检索结果">
          <header className={styles.resultsHeader}>
            <div>
              <h2>{result.query}</h2>
              <p>
                {result.hits.length
                  ? `找到 ${groups.length} 个技能`
                  : "没有找到相关内容。试着描述具体任务，或更新技能索引后再提问。"}
              </p>
            </div>
            {!result.hits.length && (
              <Button onClick={() => textarea.current?.focus()}>修改问题</Button>
            )}
          </header>
          {result.warning && (
          ...
          {phase === "idle" && result.hits.length > 0 && !answer && !answerError && (
            <Button onClick={() => void retryAnswer()}>整理中文回答</Button>
          )}
          {answer && (
            <article className={styles.answer}>
              <h3>中文回答</h3>
              <SkillMarkdown content={answer} />
            </article>
          )}
          {answerError && (
            <div role="alert" className={styles.error}>
              <span>{answerError}。下方来源仍可阅读。</span>
              <Button disabled={busy} onClick={() => void retryAnswer()}>
                重试回答
              </Button>
            </div>
          )}
          <div className={styles.hits}>
            {groups.map((group) => (
              <article className={styles.hit} key={group.skillId}>
                <header>
                  <h3>
                    <Link to="/library" search={{ skill: group.skillId }}>
                      {group.name}
                    </Link>
                  </h3>
                  <Button
                    variant="ghost"
                    disabled={!managedSkills.some((skill) => skill.id === group.skillId)}
                    onClick={() => setLinkId(group.skillId)}
                  >
                    <Link2 size={14} />
                    加入项目
                  </Button>
                </header>
                <Disclosure title={`查看来源 · ${group.sources.length} 个片段`}>
                  {group.sources.map(({ hit, index: hitIndex }) => (
                    <div key={hitIndex}>
                      <p className={styles.source}>
                        <span>
                          [{hitIndex + 1}] 第 {hit.line_start}–{hit.line_end} 行
                        </span>
                        <code>{hit.path}</code>
                      </p>
                      <pre className={styles.excerpt}>{hit.text}</pre>
                    </div>
                  ))}
                </Disclosure>
              </article>
