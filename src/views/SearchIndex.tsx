...
export function SearchIndex() {
  const { status, loading, building, error, completedAt, refresh, build } = useSkillIndex();
  return <div className={styles.page}>
    ...
    <section className={styles.panel} aria-labelledby="index-state-title">
      <div className={styles.heading}><div><h2 id="index-state-title">检索状态</h2><p role="status">{building ? "正在构建索引" : loading ? "正在检查状态…" : status?.ready ? `索引就绪 · ${status.files} 个文件` : "尚未建立可用索引"}</p></div>
        <Button variant="primary" disabled={building || loading || !status?.available} onClick={() => build()}><RefreshCw size={16} />{building ? "构建中…" : status?.ready ? "更新索引" : "建立索引"}</Button></div>
      ...
      {(error || status?.error) && <p role="alert" className={styles.error}>{error || status?.error}<Button disabled={building || loading} onClick={() => refresh()}>重试检查</Button></p>}
    ...
    <Disclosure title="索引范围与运行方式"><div className={styles.help}><Button disabled={building || loading} onClick={() => refresh()}>重新检查环境</Button><p>索引当前目录内非隐藏、非软链接的 Markdown 文档，单个文件不超过 1 MiB。模型、文档缓存和索引保存在应用缓存中，不修改技能源文件。</p><p>本地模型：{status?.model || "local/multilingual-e5-small"}。已建立索引后，提问前会自动同步文档变化；你也可以独立运行更新。</p><p>构建索引不需要 Codex CLI。中文回答另由已配置的 Codex CLI 整理。</p></div></Disclosure>
  </div>;
}
