...
export function PresetManager({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { presets, viewedPreset, setViewedPresetId, refreshPresets, refreshManagedSkills } =
    useApp();
  ...
  async function move(index: number, direction: -1 | 1) {
    ...
    try {
      await mutate(() => api.reorderPresets(next.map((preset) => preset.id)));
    } catch (failure) {
      setError(getErrorMessage(failure, "无法调整顺序，请重试。"));
    }
  }
  ...
  return (
    <>
      <DetailSheet
        open={open}
        title="技能预设"
        description="把常用技能组织为预设，在项目或工具目录中一起启用。"
        onClose={onClose}
        closeDisabled={busy}
      >
        ...
        {!presets.length && (
          <p className={styles.empty}>还没有预设。创建后，可在维护与更新中为它添加技能。</p>
        )}
            ...
            return (
              <li key={preset.id} className={styles.row}>
                ...
                <div className={styles.content}>
                  ...
                  <span>
                    {preset.skill_count} 个技能{viewedPreset?.id === preset.id ? " · 当前查看" : ""}
                  </span>
                </div>
                <Button disabled={busy} onClick={() => view(preset)}>
                  查看技能
                </Button>
                <CardActionMenu
                  label={`${preset.name} 的操作`}
                  actions={[
                    {
                      key: "rename",
                      label: "修改名称与图标",
      ...
      <CreatePresetDialog
        ...
        onCreate={async (name, description, icon) => {
          await mutate(() => api.createPreset(name, description, icon));
          toast.success("预设已创建");
        }}
      />
      <RenamePresetDialog
        ...
        currentIcon={renaming?.icon}
        onClose={() => setRenaming(null)}
        onRename={async (name, icon) => {
          if (!renaming) return;
          await mutate(() =>
            api.updatePreset(renaming.id, name, renaming.description || undefined, icon),
          );
          toast.success("预设已更新");
        }}
      />
      <ConfirmDialog
        open={open && !!deleting}
        title="删除预设"
        message={`删除“${deleting?.name ?? ""}”的分组。全局技能文件会保留。`}
