...
export function SkillLinkDialog({
  skills,
  onClose,
}: {
  skills: ManagedSkill[];
  ...
  const pendingSkills = skills.filter(
    (skill) => !results?.some((result) => result.skill_id === skill.id && result.ok),
  );
  async function deploy() {
    ...
    setError("");
    try {
      const next = await deployWorkbenchSkills(
        target.id,
      ...
      setResults((previous) =>
        skills.flatMap((skill) => {
          const result =
            next.find((item) => item.skill_id === skill.id) ??
            previous?.find((item) => item.skill_id === skill.id);
  ...
  return (
    <DetailSheet
      ...
      closeDisabled={busy}
      title="加入项目"
      description={`已选 ${skills.length} 个技能，通过软链接引用全局源文件。`}
      onClose={() => {
        if (!busy) onClose();
      }}
    >
      <div className={styles.form}>
        ...
        {normalProjects.length ? (
          <>
            <label htmlFor="link-project">目标项目</label>
            <select
              id="link-project"
              disabled={busy}
              value={projectId}
            ...
            <p className="ds-path">{target?.path}</p>
            <label htmlFor="link-agent">使用工具</label>
            <select
              id="link-agent"
              disabled={busy}
              value={agent}
            ...
            <p className="ds-note">
              全局技能更新后，项目引用会使用新内容。移出项目只解除链接，已有同名文件不会被覆盖。
            </p>
          </>
        ) : (
        ...
        {results && (
          <div className="ds-deploy-results" role="status">
            {results.map((r) => (
              <div key={r.skill_id} className={r.ok ? "is-success" : "is-error"}>
                {r.ok ? <Check size={16} /> : <AlertCircle size={16} />}
                <span>
                  <strong>{skills.find((s) => s.id === r.skill_id)?.name}</strong>
                  <small>{r.ok ? "软链接已就绪" : r.error}</small>
                </span>
              </div>
            ))}
          </div>
        ...
        <footer className="ds-dialog-actions">
          ...
          >
            {results?.length && results.every((r) => r.ok) ? "完成" : "取消"}
          </Button>
          {!busy && target && results?.length && !pendingSkills.length ? (
            <Link
            ...
            <Button
              variant="primary"
              disabled={busy || !target || !pendingSkills.length}
              busy={busy}
              onClick={() => void deploy()}
            >
              <Link2 size={15} />
