...
export function Sidebar() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("skillharbor:tool-order");
  ...
  useEffect(() => {
    const stored = localStorage.getItem("skillharbor:lobster-tool-order");
    const storedOrder: string[] = stored ? JSON.parse(stored) : [];
    const sorted = [
      ...storedOrder.flatMap((key) => {
  ...
  const handleToolDragEnd = (category: ToolCategory) => (result: DropResult) => {
    ...
    if (category === "lobster") {
      ...
      localStorage.setItem("skillharbor:lobster-tool-order", JSON.stringify(reordered.map((t) => t.key)));
    } else {
      setOrderedCodingTools(reordered);
      localStorage.setItem("skillharbor:tool-order", JSON.stringify(reordered.map((t) => t.key)));
    }
  };
