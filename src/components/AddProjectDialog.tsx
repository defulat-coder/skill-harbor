...
export function AddProjectDialog({ open, onClose, onAdded }: Props) {
  ...
  const handleAddSelected = async () => {
    ...
    try {
      ...
      if (failed.length) {
        ...
        setError(
          `已处理所选项目，${failed.length} 个目录未能添加。请检查目录或是否已经存在，再重试。`,
        );
      } else onClose();
    } catch (error) {
      setError(getErrorMessage(error, t("common.error")));
  ...
  return (
    <DetailSheet
      ...
      closeDisabled={adding || scanning}
      onClose={onClose}
    >
      {error && (
        <p role="alert" className="wb-error mb-4">
          {error}
        </p>
      )}
      {/* Tabs */}
        ...
        {(["manual", "scan", "linked"] as const).map((key) => (
          <button
            ...
            className={cn(
              ...
              tab === key ? "bg-surface text-primary shadow-xs" : "text-muted hover:text-secondary",
            )}
          >
            {t(
              key === "manual"
      ...
      {tab === "manual" ? (
        <div className="space-y-3">
          ...
          <Button onClick={handleSelectFolder} busy={adding}>
        ...
        <div className="space-y-3">
          <div className="flex gap-2">
            ...
            >
              {scanning ? t("project.scanning") : <Search className="w-4 h-4" />}
            </Button>
          </div>

          {scanned && scanResults.length === 0 && (
            <p className="text-[13px] text-muted py-4 text-center">{t("project.scanNoResult")}</p>
          )}
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              iconOnly
              aria-label={t("project.selectSkillsDir")}
              title={t("project.selectSkillsDir")}
            >
              <FolderOpen className="w-4 h-4" />
            </Button>
          </div>
          <p className="text-[12px] leading-5 text-muted">{t("project.linkedDisabledPathHint")}</p>
          <Button
