...
export function Layout() {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(() => {
    try {
      return localStorage.getItem("workbench.sidebarOpen") === "true";
    } catch {
      return false;
    }
  });
  function toggleSidebar() {
    setSidebarOpen((value) => {
      try {
        localStorage.setItem("workbench.sidebarOpen", String(!value));
      } catch {
  ...
  return (
    <div className="ds-shell">
      ...
      <header
        className="ds-titlebar"
        onMouseDown={(e) => {
          if (e.target === e.currentTarget) onDrag(e);
        }}
      >
        <button
          type="button"
        ...
        <Link to="/" className="ds-icon-button" aria-label="回到首页">
          <Home size={16} />
        </Link>
        <ChevronRight size={12} className="ds-titlebar-separator" />
        <span>{pageTitle}</span>
        ...
        <button
          ...
          className="ds-icon-button"
          aria-label="快速查找（⌘K）"
          onClick={() => window.dispatchEvent(new Event("workbench:search"))}
        >
          <Search size={16} />
        </button>
      </header>
