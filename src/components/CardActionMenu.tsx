...
export function CardActionMenu({ actions, label, className = "", onOpenChange }: Props) {
  ...
  const close = useCallback(
    async (restore = true) => {
      ...
      if (menu.current && !matchMedia("(prefers-reduced-motion: reduce)").matches) {
        try {
          await menu.current.animate([{ opacity: 1 }, { opacity: 0 }], {
            duration: 140,
  ...
  useEffect(() => {
    ...
    const outside = (e: PointerEvent) => {
      if (!(e.target instanceof Node) || !container.current?.contains(e.target)) void close(false);
    };
      ...
      {open && (
        <div
          ...
          onKeyDown={(e) => {
            ...
            const index = active instanceof HTMLButtonElement ? nodes.indexOf(active) : -1;
            if (e.key === "Escape") {
              e.preventDefault();
              e.stopPropagation();
              void close();
        ...
        >
          {actions.map((action) => (
            <button
              ...
              onClick={() => {
                void close().then(() => action.onSelect());
              }}
            >
