...
const POLICY = { globs: ['**/*.md', '**/*.mdx'], follow: false, hidden: false, noIgnore: true, maxFileSizeBytes: MAX_FILE };
...
async function run(request) {
  ...
  if (typeof request.root !== 'string' || !path.isAbsolute(request.root) || typeof request.cacheDir !== 'string' || !path.isAbsolute(request.cacheDir)) throw new Error('root 和 cacheDir 必须为绝对路径');
  const root = await fs.realpath(request.root);
  if (!(await fs.stat(root)).isDirectory()) throw new Error('技能根路径不是目录');
  if (inside(root, path.resolve(request.cacheDir)) || inside(path.resolve(request.cacheDir), root)) throw new Error('索引缓存必须与技能源目录分离');
  await fs.mkdir(request.cacheDir, { recursive: true });
  const cacheDir = await fs.realpath(request.cacheDir);
  if (inside(root, cacheDir) || inside(cacheDir, root)) throw new Error('索引缓存必须与技能源目录分离');
  ...
  const priorRoot = await fs.readFile(marker, 'utf8').catch(e => { if (e.code === 'ENOENT') return null; throw e; });
  ...
  const service = await createZvecGrep({ root: workspace, home: cacheDir, embedding: MODEL, device: 'cpu', modelCacheDir: path.join(cacheDir, 'models') });
  try {
    const status = async () => {
      ...
      return { root, available: true, ready: info.indexed && failed === 0 && pending === 0, model: MODEL,
        files: info.status?.filesIndexed ?? 0,
        ...(failed || pending ? { error: `${failed} 个文件失败，${pending} 个文件待处理` } : {}) };
    };
    if (request.op === 'status') return await status();
    if (request.op === 'query' && (typeof request.query !== 'string' || !request.query.trim() || request.query.length > 4000)) throw new Error('请输入不超过 4000 字的检索问题');
    const files = await synchronize(root, workspace);
    const info = await service.info();
    if (request.op === 'index' || !info.indexed) {
      const result = await service.index(POLICY);
      if (result.filesFailed || result.filesPending) throw new Error(`索引未完成：${result.filesFailed} 个文件失败，${result.filesPending} 个文件待处理`);
    }
    if (request.op === 'index') return { ...(await status()), files };
    const result = await service.context({ ...POLICY, query: request.query.trim(), autoUpdate: true, limit: 12 });
    ...
    for (const item of result.items) {
      ...
      hits.push({ path: original, line_start: range.kind === 'text' ? range.startLine : 1,
        line_end: range.kind === 'text' ? range.endLine : 1, text: item.content, score: item.score ?? 0 });
    }
    return { query: request.query.trim(), hits, ...(!files ? { warning: '中央技能目录中没有可索引的 Markdown 文件' } : {}) };
...
try {
  ...
  for await (const chunk of process.stdin) { input += chunk; if (input.length > 32000) throw new Error('请求过大'); }
