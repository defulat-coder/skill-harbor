...
function call(op, query) {
  const run = spawnSync(process.execPath, [runtime], { input: JSON.stringify({ op, query, root, cacheDir }), encoding: 'utf8', timeout: 180000, maxBuffer: 4000000 });
...
await fs.writeFile(path.join(root, 'audio/SKILL.md'), '# Audio transcription\nConvert spoken audio recordings to written text transcripts. Transcribe meetings and identify speaker names.\n');
assert.match(call('query', '把会议录音转成文字').hits[0].path, /audio\/SKILL.md$/);
await fs.writeFile(path.join(root, 'frontend/SKILL.md'), '# Image background removal\nRemove the background from photographs and export transparent PNG product images.\n');
assert.match(call('query', '去掉照片背景生成透明图片').hits[0].path, /frontend\/SKILL.md$/);
assert.match(call('query', '去掉照片背景').hits.find(h => /frontend/.test(h.path)).text, /background removal/);
await fs.rm(path.join(root, 'database'), { recursive: true });
assert.equal(call('query', '数据库查询优化').hits.some(h => /database/.test(h.path)), false);
assert.equal(await fs.stat(path.join(root, '.zvec-grep')).then(() => true, () => false), false);
await fs.writeFile(path.join(base, 'smoke-results.json'), JSON.stringify(evidence, null, 2));
console.log(JSON.stringify({ ok: true, checks: 11, evidence: path.join(base, 'smoke-results.json') }));
