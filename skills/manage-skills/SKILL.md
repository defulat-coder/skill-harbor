---
name: manage-skills
description: Manage the user's shared agent-skill library via skillharbor-cli — install, update, remove, deploy or undeploy skills per agent, manage presets, organize tags, search, and adopt existing skills. Use this whenever the user wants Claude Code, Codex, Cursor, or another agent to gain or lose a skill, wants to organize the central library, or asks what is installed or deployed. Prefer this over direct agent-folder installs because SkillHarbor preserves source metadata, preset membership, updates, and cross-agent deployment state.
---
...
1. **Resolve the CLI first, then use the path it prints.** Run this once (POSIX
   ...
   ```bash
   D="$HOME/.skillharbor/bin"
   B="$D/skillharbor-cli"; [ -e "$B" ] || B="$B.exe"   # .exe on Windows
   if [ -s "$D/.version" ] && [ -x "$B" ]; then
     echo "$B"
   elif [ -s "$D/.version" ] || [ -e "$B" ]; then
     echo BRIDGE_BROKEN
   else
     P="$(command -v skillharbor-cli 2>/dev/null || true)"
     [ -x "$P" ] && echo "$P"
   fi
   ```
   ...
   **Substitute the printed path into every command below**, wherever the
   ...
   - **A path under `~/.skillharbor/bin`** — the desktop app published this
   ...
   - **`BRIDGE_BROKEN`** — something the app left behind is here but does not
     ...
     the SkillHarbor app once, which republishes it.
   ...
   find-skills, or tell the user to install SkillHarbor.
...
There's **one central library** at `~/.skillharbor/skills/` that all agents share. Each skill has source metadata, preset membership, tags, and zero or more real deployments in agent directories. A **preset** is a reusable group; several presets may be deployed at the same time.

Keep these three states separate:
- **Library**: install/remove controls whether SkillHarbor owns the skill.
