...
fn build_tray_menu_from_data<R: tauri::Runtime>(
    ...
    let app_name = MenuItem::with_id(app, "tray-app-name", "SkillHarbor", false, None::<&str>)?;
    ...
    let show_item = MenuItem::with_id(app, "show", "Open SkillHarbor", true, None::<&str>)?;
...
pub fn run() {
    ...
    if let Some(path) = std::env::var_os("SKILLHARBOR_SANDBOX_DIR")
        .or_else(|| std::env::var_os("SKILLS_MANAGER_SANDBOX_DIR"))
    {
        core::central_repo::set_development_home(std::path::PathBuf::from(path));
    }
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // agents can drive SkillHarbor without it being on PATH. A
