...
// Existing intermediate symlinks must not redirect a project deployment outside its root.
pub(crate) fn ensure_project_skills_root(
    project: &ProjectRecord,
    target_root: &Path,
) -> Result<(), AppError> {
    let root = Path::new(&project.path).canonicalize()?;
    if !target_root.starts_with(Path::new(&project.path)) {
        return Err(AppError::invalid_input("技能目录不在项目内"));
    }
    let mut ancestor = target_root;
    loop {
        if std::fs::symlink_metadata(ancestor).is_ok() {
            if !ancestor.canonicalize()?.starts_with(&root) {
                return Err(AppError::invalid_input(
                    "技能目录链接指向项目外，请先在设置中检查路径",
                ));
            }
            return Ok(());
        }
        ancestor = ancestor
            .parent()
            .ok_or_else(|| AppError::invalid_input("无效项目路径"))?;
    }
}

/// Normal project membership is a reference, regardless of global sync preferences.
/// Linked workspaces represent global tool directories and retain their legacy mode.
fn add_workspace_skill(
    store: &SkillStore,
    project: &ProjectRecord,
    agent: &str,
    source: &Path,
    ...
    if project.workspace_type != "linked" {
        ...
        sync_engine::sync_skill(source, target, mode, sync_engine::ReplacePolicy::NoClobber)
            .map_err(AppError::io)?;
    }
    Ok(())
}
...
pub async fn export_skill_to_project(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        for agent_key in &agent_keys {
            ...
            if project.workspace_type != "linked" {
                ensure_project_skills_root(&project, &skills_root)?;
            }
            ...
            if std::fs::symlink_metadata(&target_dir).is_ok()
                || disabled_root
                    .as_ref()
                    .map(|path| std::fs::symlink_metadata(path.join(&dir_name)).is_ok())
                    .unwrap_or(false)
            {
        ...
        for agent_key in &agent_keys {
            ...
            add_workspace_skill(&store, &project, agent_key, &source, &target_dir)?;
        }
...
pub async fn update_project_skill_from_center(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let configured_mode = store.get_setting("sync_mode").map_err(AppError::db)?;
        let mode = if record.workspace_type == "linked" {
            sync_engine::sync_mode_for_tool(&agent, configured_mode.as_deref())
        } else if std::fs::symlink_metadata(&target_path)?
            .file_type()
            .is_symlink()
        {
            // Existing project references already read current central content. Do not
            // convert them to copies because the user's global preference is copy.
            sync_engine::link_project_skill(&source, &target_path).map_err(AppError::io)?;
            return Ok(());
        } else {
            // An explicit refresh of a legacy copy remains a copy; merely reading or
            // adding project skills never converts or deletes that existing directory.
...
mod tests {
    ...
    fn reference_project(path: &std::path::Path, workspace_type: &str) -> super::ProjectRecord {
        super::ProjectRecord {
            id: "project".into(),
            name: "test".into(),
    ...
    #[cfg(unix)]
    #[test]
    fn ordinary_add_links_even_when_global_setting_is_copy() {
        let tmp = tempdir().unwrap();
        let store = super::SkillStore::new(&tmp.path().join("test.db")).unwrap();
        store.set_setting("sync_mode", "copy").unwrap();
        let source = tmp.path().join("source");
        fs::create_dir(&source).unwrap();
        fs::write(source.join("SKILL.md"), "one").unwrap();
        let project = reference_project(&tmp.path().join("project"), "project");
        let target = std::path::Path::new(&project.path).join(".codex/skills/example");
        super::add_workspace_skill(&store, &project, "codex", &source, &target).unwrap();
        ...
        assert_eq!(fs::read_to_string(target.join("SKILL.md")).unwrap(), "two");
        assert_eq!(
    ...
    #[test]
    fn ordinary_add_preserves_same_named_real_directory() {
        let tmp = tempdir().unwrap();
        let store = super::SkillStore::new(&tmp.path().join("test.db")).unwrap();
        let source = tmp.path().join("source");
        fs::create_dir(&source).unwrap();
        fs::write(source.join("SKILL.md"), "central").unwrap();
        let project = reference_project(&tmp.path().join("project"), "project");
        let target = std::path::Path::new(&project.path).join(".codex/skills/example");
        fs::create_dir_all(&target).unwrap();
        fs::write(target.join("SKILL.md"), "local edits").unwrap();
        assert!(super::add_workspace_skill(&store, &project, "codex", &source, &target).is_err());
        assert_eq!(
    ...
    #[test]
    fn linked_global_workspace_keeps_copy_preference() {
        let tmp = tempdir().unwrap();
        let store = super::SkillStore::new(&tmp.path().join("test.db")).unwrap();
        store.set_setting("sync_mode", "copy").unwrap();
        let source = tmp.path().join("source");
        fs::create_dir(&source).unwrap();
        fs::write(source.join("SKILL.md"), "central").unwrap();
        let project = reference_project(&tmp.path().join("global-tool-skills"), "linked");
        let target = std::path::Path::new(&project.path).join("example");
        super::add_workspace_skill(&store, &project, "codex", &source, &target).unwrap();
        assert!(!fs::symlink_metadata(&target)
            .unwrap()
            .file_type()
