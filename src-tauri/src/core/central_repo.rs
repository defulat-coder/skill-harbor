...
/// Data directory names under the user's home. The legacy one belongs to the
/// pre-rebrand app; it is migrated (renamed) on first access, never abandoned.
const HOME_DIR_NAME: &str = ".skillharbor";
const LEGACY_HOME_DIR_NAME: &str = ".skills-manager";

/// Isolated data root for desktop end-to-end checks; unavailable in release builds.
#[cfg(debug_assertions)]
pub fn set_development_home(path: PathBuf) {
    *HOME_DIR_OVERRIDE.get_or_init(|| Mutex::new(None)).lock().unwrap() = Some(path.clone());
    set_runtime_base_dir_override(Some(path.join(HOME_DIR_NAME)));
}
...
/// `~/.skillharbor`, ignoring any configured relocation.
///
...
pub fn home_base_dir() -> PathBuf {
    if let Some(path) = HOME_DIR_OVERRIDE
        ...
        .clone()
    {
        return path.join(HOME_DIR_NAME);
    }
    let new = dirs::home_dir()
        .expect("Cannot determine home directory")
        .join(HOME_DIR_NAME);
    migrate_legacy_home_dir(new)
}

/// One-time rename of the pre-rebrand data directory (`~/.skills-manager` →
/// `~/.skillharbor`). The rename carries everything with it — DB, config,
/// local-workbench, the CLI bridge — so no per-file migration is needed. On
/// failure the old path keeps serving this session and the rename is retried
/// on the next launch.
fn migrate_legacy_home_dir(new: PathBuf) -> PathBuf {
    if new.exists() {
        return new;
    }
    let legacy = new.with_file_name(LEGACY_HOME_DIR_NAME);
    if !legacy.exists() {
        return new;
    }
    match fs::rename(&legacy, &new) {
        ...
        Err(err) => {
            record_startup_error(format!(
                "central repo: could not rename data directory {} to {} ({err}); \
                 using the old location this session",
                legacy.display(),
                new.display()
            ));
            legacy
...
fn config_file_path() -> PathBuf {
    let root = dirs::config_dir().unwrap_or_else(default_base_dir);
    let new = root.join("skillharbor").join(CONFIG_FILE_NAME);
    if new.exists() {
        return new;
    }
    let legacy = root.join("skills-manager").join(CONFIG_FILE_NAME);
    if !legacy.exists() {
        return new;
    }
    // Carry a pre-rebrand config over so a customized central path survives
    // the rename. On failure keep reading the old file — losing it would
    // silently revert the user to the default library (#228 territory).
    if let Some(parent) = new.parent() {
        if fs::create_dir_all(parent).is_ok() && fs::rename(&legacy, &new).is_ok() {
            return new;
        }
    }
    legacy
...
pub fn db_path() -> PathBuf {
    let base = base_dir();
    let new = base.join("skillharbor.db");
    if new.exists() {
        return new;
    }
    let legacy = base.join("skills-manager.db");
    if !legacy.exists() {
        return new;
    }
    // A pre-rebrand database. This also covers customized central directories,
    // which the home-dir rename never reaches. Rename any WAL/SHM sidecars
    // along with the main file so an unclean shutdown's journals stay valid.
    if let Err(err) = fs::rename(&legacy, &new) {
        record_startup_error(format!(
            "central repo: could not rename database {} to {} ({err}); using the existing file",
            legacy.display(),
            new.display()
        ));
        return legacy;
    ...
    for suffix in ["-wal", "-shm"] {
        let old_sidecar = base.join(format!("skills-manager.db{suffix}"));
        if old_sidecar.exists() {
            let _ = fs::rename(&old_sidecar, base.join(format!("skillharbor.db{suffix}")));
        }
    }
    new
}
