...
const METADATA_DIR_NAME: &str = ".skillharbor";
/// Pre-rebrand name of the sync-metadata directory, migrated on first access.
const LEGACY_METADATA_DIR_NAME: &str = ".skills-manager";

pub fn metadata_dir() -> PathBuf {
    let skills = central_repo::skills_dir();
    let new = skills.join(METADATA_DIR_NAME);
    if new.exists() {
        return new;
    }
    let legacy = skills.join(LEGACY_METADATA_DIR_NAME);
    if !legacy.exists() {
        return new;
    }
    // Carry pre-rebrand sync metadata over on first access; on failure keep
    // using the old directory this session and retry on the next one.
    match fs::rename(&legacy, &new) {
        Ok(()) => new,
        Err(err) => {
            log::warn!(
                "sync metadata: could not rename {} to {} ({err}); using the old directory",
                legacy.display(),
                new.display()
            );
            legacy
...
fn write_schema() -> Result<()> {
    let schema = SchemaFile {
        ...
        created_by: "skillharbor".to_string(),
    };
...
fn central_repo_has_valid_skill_dirs() -> Result<bool> {
    ...
    for entry in WalkDir::new(&skills_dir)
        ...
        .filter_entry(|entry| {
            let name = entry.file_name().to_string_lossy();
            name != ".git" && name != METADATA_DIR_NAME && name != LEGACY_METADATA_DIR_NAME
        })
    {
        let entry = entry?;
