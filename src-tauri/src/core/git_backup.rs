...
/// Delete `refs/skillharbor/*` (and pre-rebrand `refs/skills-manager/*`)
/// copies that a mirror / push-all style operation uploaded to the remote
/// (merge-engine design §11-2). The app's own push never sends them; this
/// cleans up after manual advanced git use. Local refs under either namespace
/// are functional (conflict pins, recovery anchors) and stay untouched.
/// Returns the number of remote refs removed.
pub fn prune_hidden_refs_on_remote(skills_dir: &Path) -> Result<usize> {
    ...
    let listed = run_git_env(
        skills_dir,
        &["ls-remote", "origin", "refs/skillharbor/*", "refs/skills-manager/*"],
        &env,
    )?;
...
/// SkillHarbor process attempting to populate the target between check
...
/// output, skipping dot-entries (`.skillharbor` metadata, `.gitignore`).
...
const OVERSIZED_SECTION_BEGIN: &str = "# skillharbor: oversized skills excluded from backup (auto-managed)";
const OVERSIZED_SECTION_END: &str = "# skillharbor: end oversized skills";
/// Pre-rebrand markers of the same managed section, recognized on read so the
/// section is rewritten in place instead of duplicated.
const LEGACY_OVERSIZED_SECTION_BEGIN: &str = "# skills-manager: oversized skills excluded from backup (auto-managed)";
...
pub(crate) fn apply_oversized_exclusions(skills_dir: &Path, limit: u64) -> Result<Vec<String>> {
    ...
    if !dirs.is_empty() {
        ...
        let meta_dir = {
            let new = skills_dir.join(".skillharbor/skills");
            if new.is_dir() {
                new
            } else {
                skills_dir.join(".skills-manager/skills")
            }
        };
        if meta_dir.is_dir() {
        ...
        for (rel, bytes) in dirs {
            ...
            if let Some(id) = id_by_path.get(&rel) {
                lines.push(format!("/.skillharbor/skills/{}.json", gitignore_escape(id)));
            }
            log::info!(
...
/// namespace of THIS repo's working tree (current and pre-rebrand name).
/// Best-effort; a leftover appears only when a writer crashed mid-write.
fn remove_tmp_metadata_files(skills_dir: &Path) {
    for name in [".skillharbor", ".skills-manager"] {
        let meta_root = skills_dir.join(name);
        if !meta_root.exists() {
            continue;
        }
...
mod tests {
    ...
    fn device_email_slugs_name_with_fallback() {
        assert_eq!(device_email("MacBook Pro"), "macbook-pro@skillharbor.local");
        assert_eq!(device_email("公司 Windows"), "windows@skillharbor.local");
        assert_eq!(device_email("公司"), "device@skillharbor.local");
    }
    ...
    fn oversized_untracked_skill_is_excluded_but_tracked_one_is_not() {
        ...
        let meta_dir = dir.join(".skillharbor/skills");
        ...
        assert!(gitignore.contains("/.skillharbor/skills/skill-huge.json"), "{gitignore}");
        ...
        assert!(run_git(dir, &["cat-file", "-e", "HEAD:.skillharbor/skills/skill-huge.json"]).is_err());
    ...
    fn app_commits_carry_protocol_marker_and_trailer() {
        ...
        run_git(dir, &["cat-file", "-e", "HEAD:.skillharbor/protocol.json"]).unwrap();
    ...
    fn count_changed_top_dirs_skips_metadata_and_root_dotfiles() {
        let porcelain = " M .skillharbor/skills/x.json\n M .gitignore\n";
