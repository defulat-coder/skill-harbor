...
const KEYRING_SERVICE: &str = "skillharbor-git-backup";
/// Pre-rebrand keychain service. Existing entries are still read (and removed
/// on delete), but credentials are only ever written under the new service.
const LEGACY_KEYRING_SERVICE: &str = "skills-manager-git-backup";
...
const ENV_USERNAME: &str = "SKILLHARBOR_ASKPASS_USERNAME";
const ENV_PASSWORD: &str = "SKILLHARBOR_ASKPASS_PASSWORD";
...
fn legacy_keyring_entry(host: &str) -> Result<keyring::Entry> {
    keyring::Entry::new(LEGACY_KEYRING_SERVICE, host).context("Failed to open keychain entry")
}
...
pub fn load_credential(host: &str) -> Result<Option<RemoteCredential>> {
    let parse = |payload: String| {
        serde_json::from_str(&payload).with_context(|| format!("Corrupted keychain entry for {host}"))
    };
    match keyring_entry(host)?.get_password() {
        Ok(payload) => Ok(Some(parse(payload)?)),
        Err(keyring::Error::NoEntry) => {
            // Pre-rebrand keychain entries still authenticate the same remote.
            match legacy_keyring_entry(host)?.get_password() {
                Ok(payload) => Ok(Some(parse(payload)?)),
...
pub fn delete_credential(host: &str) -> Result<()> {
    ...
    for entry in [keyring_entry(host)?, legacy_keyring_entry(host)?] {
        match entry.delete_credential() {
            Ok(()) => removed = true,
            Err(keyring::Error::NoEntry) => {}
            Err(e) => {
                return Err(e).with_context(|| format!("Failed to delete git credential for {host}"));
            }
        }
    }
    if removed {
...
# Managed by SkillHarbor. Supplies git credentials from the environment.\n\
case \"$1\" in\n\
  *[Uu]sername*) printf '%s\\n' \"${SKILLHARBOR_ASKPASS_USERNAME}\" ;;\n\
  *) printf '%s\\n' \"${SKILLHARBOR_ASKPASS_PASSWORD}\" ;;\n\
esac\n";
