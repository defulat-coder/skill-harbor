...
impl ReplacePolicy<'_> {
    ...
    fn describe(&self) -> &'static str {
        match self {
            ReplacePolicy::NoClobber => "is not managed by SkillHarbor",
...
/// Add a project reference without replacing files or degrading to a copy.
/// Global agent deployment deliberately continues to use `sync_skill` and its
/// platform fallbacks. Ordinary project membership has a stricter contract.
pub fn link_project_skill(source: &Path, target: &Path) -> Result<SyncMode> {
    if !source.is_dir() {
        anyhow::bail!("Skill source directory {:?} does not exist", source);
    }
    let state = authorize_replacement(source, target, ReplacePolicy::NoClobber)?;
    if let Some(parent) = target.parent() {
        std::fs::create_dir_all(parent)?;
    }
    ensure_dst_not_inside_src(source, target)?;
    if state == TargetState::LinkToSource {
        return Ok(SyncMode::Symlink);
    }
    // Creating the link atomically fails if another writer claimed this path.
    // Never remove a target here, even after a successful preflight.
    crate::core::file_watcher::mute_self_writes(target);
    let source = source.canonicalize()?;
    #[cfg(unix)]
    std::os::unix::fs::symlink(&source, target).with_context(|| {
        format!(
            "Cannot create project symlink {:?}; no copy was created",
            target
        )
    })?;
    #[cfg(windows)]
    std::os::windows::fs::symlink_dir(&source, target)
        .with_context(|| format!("Cannot create project symlink {:?}; enable Developer Mode or symlink permission. No copy was created", target))?;
    #[cfg(not(any(unix, windows)))]
    anyhow::bail!("Project symlinks are unsupported on this platform; no copy was created");
    Ok(SyncMode::Symlink)
}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn project_link_refuses_dangling_link_without_replacing_it() {
        ...
        assert!(link_project_skill(&source, &target).is_err());
        assert_eq!(std::fs::read_link(&target).unwrap(), missing);
    }

    #[test]
    fn project_link_rejects_missing_source_without_creating_target() {
        let tmp = tempfile::tempdir().unwrap();
        let target = tmp.path().join("project/target");
        assert!(link_project_skill(&tmp.path().join("missing"), &target).is_err());
