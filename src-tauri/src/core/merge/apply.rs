...
use super::snapshot::{self, FileEntry, LEGACY_METADATA_DIR, METADATA_DIR, Snapshot, skill_identical};
...
pub fn object_merge_pull_unlocked(store: &SkillStore, skills_dir: &Path) -> Result<MergeSummary> {
    ...
    // current line-merge behavior. The marker may sit at either the current
    // or the pre-rebrand path.
    let theirs_tree = repo.find_commit(theirs)?.tree()?;
    if !tree_has_protocol_marker(&theirs_tree) {
...
fn check_old_client_writes(
    ...
    if !doubles.is_empty() {
        bail!(
            "sync blocked: an old app version performed a line-level merge on another device — commit(s) {}. Upgrade that device and retry, or use the recovery flow (a safety snapshot is taken first)",
            doubles.join("; ")
        );
    }
    ...
    for (label, tip, violated) in [
        ...
        validate::validate_input_tip(repo, &tree, &tolerated).with_context(|| {
            format!(
                "sync blocked: an old app version wrote to the {label} library and left it inconsistent — upgrade that device and repair via the recovery flow"
            )
        })?;
    ...
    let listed: Vec<String> = ours_violations
        ...
        .collect();
    Ok(Some(format!(
        "old app version wrote commit(s) {} — please upgrade that device",
        listed.join("; ")
    )))
}
...
fn device_for_skill(
    ...
    if let Some(info) = touch.get(&meta_path).or_else(|| {
        touch.get(&format!("{LEGACY_METADATA_DIR}/skills/{skill_id}.json"))
    }) {
...
fn plan_to_edits(
    ...
    // A pre-rebrand metadata namespace is fully superseded by the metadata
    // rebuilt under `METADATA_DIR` below — drop the legacy tree outright.
    if ours.legacy_metadata_dir {
        edits.insert(LEGACY_METADATA_DIR.to_string(), TreeEdit::Remove);
    }
