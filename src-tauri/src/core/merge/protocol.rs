...
//! trailer and guarantees `.skillharbor/protocol.json` exists in the tree
...
//! client. (The trailer keys predate the rebrand and stay as-is: they are
//! replayed from existing commit history.)
...
pub const PROTOCOL_FILE_REL: &str = ".skillharbor/protocol.json";
/// Pre-rebrand location of the marker. Historical commits carry it there, so
/// tree-level checks must look at both paths; new files are only written at
/// [`PROTOCOL_FILE_REL`].
pub const LEGACY_PROTOCOL_FILE_REL: &str = ".skills-manager/protocol.json";
...
/// self-heals. A pre-rebrand marker at [`LEGACY_PROTOCOL_FILE_REL`] still
/// counts toward the version floor, but the file is (re)written at the
/// current path so fresh trees carry the current directory name.
pub fn ensure_protocol_file(skills_dir: &Path) -> Result<()> {
    let path = skills_dir.join(PROTOCOL_FILE_REL);
    let read_version = |p: &Path| -> Option<u32> {
        std::fs::read_to_string(p)
            .ok()
            .and_then(|raw| serde_json::from_str::<ProtocolFile>(&raw).ok())
            .map(|file| file.merge_protocol)
    };
    let current = read_version(&path);
    if current.is_some_and(|v| v >= MERGE_PROTOCOL_VERSION) {
        return Ok(());
    }
    let legacy = read_version(&skills_dir.join(LEGACY_PROTOCOL_FILE_REL));
    let merge_protocol = current
        .into_iter()
        .chain(legacy)
        .max()
        .unwrap_or_default()
        .max(MERGE_PROTOCOL_VERSION);
    ...
    std::fs::write(&path, protocol_file_bytes(&file)?)
