...
//! hidden refs under `refs/skillharbor/` only pin theirs-side objects
...
pub const CONFLICT_REF_PREFIX: &str = "refs/skillharbor/conflict/";
pub const STAGING_REF_PREFIX: &str = "refs/skillharbor/conflict-staging/";

/// Pre-rebrand namespace. Backup repositories in the wild still carry these
/// refs, so reads fall back to them and deletes cover them; writes always
/// use the new namespace.
const REF_PREFIX: &str = "refs/skillharbor/";
const LEGACY_REF_PREFIX: &str = "refs/skills-manager/";
const LEGACY_CONFLICT_REF_PREFIX: &str = "refs/skills-manager/conflict/";
const LEGACY_STAGING_REF_PREFIX: &str = "refs/skills-manager/conflict-staging/";

/// The pre-rebrand counterpart of a new-namespace ref, if it is one.
fn legacy_ref_name(name: &str) -> Option<String> {
    name.strip_prefix(REF_PREFIX)
        .map(|rest| format!("{LEGACY_REF_PREFIX}{rest}"))
}
