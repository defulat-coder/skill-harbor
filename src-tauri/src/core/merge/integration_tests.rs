...
impl Device {
    ...
    fn write_skill_full(&self, id: &str, path: &str, content: &str, enabled: bool, tags: &[&str]) {
        ...
        let meta_dir = self.skills.join(".skillharbor/skills");
        ...
        .unwrap();
        std::fs::write(
            self.skills.join(".skillharbor/schema.json"),
            b"{\n  \"schema_version\": 1,\n  \"app_min_version\": \"2.0.0\",\n  \"created_by\": \"skillharbor\"\n}\n",
        )
        .unwrap();
    }
    ...
    fn remove_skill(&self, id: &str, path: &str) {
        ...
        let meta = self.skills.join(format!(".skillharbor/skills/{id}.json"));
        if meta.exists() {
    ...
    fn meta_of(&self, id: &str) -> Option<SkillMetaFile> {
        let raw = std::fs::read_to_string(
            self.skills.join(format!(".skillharbor/skills/{id}.json")),
        )
        .ok()?;
...
fn resolve_keep_both_duplicates_under_device_suffix() {
    ...
    let meta_dir = a.skills.join(".skillharbor/skills");
...
fn legacy_dirt_does_not_brick_the_merge() {
    ...
    b.activate();
    std::fs::write(
        b.skills.join(".skillharbor/skills/skill-1.json.tmp.0000-dead-beef"),
...
fn commit_never_picks_up_tmp_metadata_leftovers() {
    ...
    a.write_skill("skill-1", "alpha", "base");
    std::fs::write(
        a.skills.join(".skillharbor/skills/skill-1.json.tmp.0000-dead-beef"),
        "leftover",
    )
    .unwrap();
    a.commit("seed");
    ...
    assert!(!a
        .skills
        .join(".skillharbor/skills/skill-1.json.tmp.0000-dead-beef")
        .exists());
}
...
fn legacy_remote_without_protocol_falls_back_to_system_merge() {
    ...
    b.activate();
    std::fs::remove_file(b.skills.join(".skillharbor/protocol.json")).unwrap();
