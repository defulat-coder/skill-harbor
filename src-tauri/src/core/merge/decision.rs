...
/// plus atomic-write temp leftovers anywhere under the metadata dir (either
/// the current or the pre-rebrand name).
pub(crate) fn is_metadata_namespace_junk(path: &str) -> bool {
    let rest = path
        .strip_prefix(".skillharbor/")
        .or_else(|| path.strip_prefix(".skills-manager/"));
