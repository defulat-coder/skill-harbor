...
use super::snapshot::{LEGACY_METADATA_DIR, MAX_SKILL_DEPTH, METADATA_DIR, tree_is_valid_skill_dir};
...
fn validate_tree(
    ...
    let meta_tree = subtree(repo, tree, METADATA_DIR)
        .or_else(|_| subtree(repo, tree, LEGACY_METADATA_DIR))
        .context("merged tree validation: metadata directory missing")?;
