...
fn worktree_meta_path(skills_dir: &Path, skill_id: &str) -> std::path::PathBuf {
    let file = format!("{skill_id}.json");
    let new = skills_dir
        .join(snapshot::METADATA_DIR)
        .join("skills")
        .join(&file);
    if !new.exists() {
        let legacy = skills_dir
            .join(snapshot::LEGACY_METADATA_DIR)
            .join("skills")
            .join(&file);
        if legacy.exists() {
            return legacy;
...
fn free_path_for(skills_dir: &Path, wanted: &str, own_id: &str) -> Result<String> {
    ...
    let meta_dir = {
        let new = skills_dir.join(snapshot::METADATA_DIR).join("skills");
        if new.is_dir() {
            new
        } else {
            skills_dir.join(snapshot::LEGACY_METADATA_DIR).join("skills")
        }
    };
    if meta_dir.is_dir() {
